...
public record ArticleWithCommentsResponse(
    ...
    private static Set<ArticleCommentResponse> organizeChildComments(Set<ArticleCommentDto> dtos) {
        Map<Long, ArticleCommentResponse> map = dtos.stream()
                .map(ArticleCommentResponse::from)  // 내부 빈 Set 에 Comparator 를 넣고 객체 생성 후 Map 으로 변환
                .collect(Collectors.toMap(ArticleCommentResponse::id, Function.identity()));

        map.values().stream()
                .filter(ArticleCommentResponse::hasParentComment) // 대댓글만 필터 후 댓글에 추가 -> 대댓글들이 넣어준 Comparator 에 의해 자동 정렬
                .forEach(comment -> {
        ...
        return map.values().stream()
                .filter(comment -> !comment.hasParentComment()) // 댓글만 필터 후 정렬
